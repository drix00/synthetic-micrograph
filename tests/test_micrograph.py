...
"""
.. py:currentmodule:: tests.test_micrograph
.. moduleauthor:: Hendrix Demers <hendrix.demers@mail.mcgill.ca>

Tests for the :py:mod:`micrograph` module.
"""
...
def test_tests_layout_matches_source():
    ...
    project_path = Path(tests_dir.parent, f"{module_name}")

    source_name = this_files_path.name.split("test_", maxsplit=1)[1]
    assert source_name[:-3] == f"{module_name}"
...
def test_source_layout_matches_tests():
    # verify that this file is - itself - in tests/
    this_files_path = Path(__file__)
    source_path = this_files_path.parent.parent / f"{module_name}"
    assert source_path.name == f"{module_name}"
